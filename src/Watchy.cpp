...
void Watchy::showMenu(byte menuIndex, bool partialRefresh) {
    ...
    uint16_t w, h;
    int16_t yPos;
    int page = floor((float)menuIndex / MENU_ITEMS_VISIBLE);
    int offset = page * MENU_ITEMS_VISIBLE;
    for (int i = 0; i < MENU_ITEMS_VISIBLE; i++) {
        ...
        if (index == menuIndex) {
            display.getTextBounds(menuItems[index], 0, yPos, &x1, &y1, &w, &h);
...
weatherData Watchy::getWeatherData() { return getWeatherData(settings.cityID, settings.weatherUnit, settings.weatherLang, settings.weatherURL, settings.weatherAPIKey, settings.weatherUpdateInterval); }
